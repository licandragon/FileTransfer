...
<template>
  <div class="relative min-h-[calc(100vh-65px)] flex items-center justify-center overflow-hidden px-4 py-8">
    ...
    <!-- 404 gigante -->
    <span aria-hidden="true" class="ghost-number">
      404
    </span>

    <!-- Contenido -->
    <div class="relative z-10 flex flex-col items-center text-center gap-5 fade-up">

      <!-- icono -->
      <div class="w-16 h-16 sm:w-18 sm:h-18 rounded-[20px]
      ...
      <!-- titulo -->
      <h1 class="text-[clamp(1.5rem,3vw,2.2rem)]
        font-extrabold tracking-[-0.03em]">
        Página no encontrada
      </h1>

      <!-- descripción -->
      <p class="text-[#5a5550] text-[0.95rem]
        flex items-center gap-2 flex-wrap justify-center
        max-w-md">
        La ruta

        <code class="bg-[rgba(200,169,110,0.1)]
          border border-[rgba(200,169,110,0.2)]
          text-[#c8a96e]
          px-2 py-0.5
      ...
      <div class="flex gap-3 flex-wrap justify-center">
        ...
        <RouterLink v-if="isAuthenticated" to="/dashboard" class="inline-flex items-center
          px-6 py-[0.65rem]
          rounded-[10px]
          text-[0.9rem] font-semibold
        ...
        <RouterLink v-else to="/login" class="inline-flex items-center
          px-6 py-[0.65rem]
          rounded-[10px]
          text-[0.9rem] font-semibold
          bg-[rgba(255,255,255,0.04)]
      ...
      <div class="flex flex-col items-center gap-3 mt-1 max-w-lg">
        ...
        <div class="flex gap-2 flex-wrap justify-center">
          ...
          <RouterLink v-for="route in availableRoutes" :key="route.path" :to="route.path" class="inline-flex items-center gap-1
            px-3 py-[0.35rem]
            rounded-full
